...
/**
 * Removes all synchronized blocks from each method in the given
 * class by generating a fresh static method in the same class per
 * synchronized block.
 * 
...
public class SyncRemover {
	...
	private Type type;
	/**
	 * Map from each monitorenter node in the current method to the
	 * set of all nodes in the corresponding synchronized block.
	 */
	 ...
	 * generated sync-free method (including the outermost one) to
	 * the corresponding node in the control-flow graph of the
	 * current method.
	 */
	private Map<Inst, Inst> visited;
	/**
	 * Current number of generated sync-free methods (excluding
	 * the outermost ones) in the given class.
	 */
	int numSyncFreeMethods;
	public SyncRemover(Type type) {
		this.type = type;
		...
		for (Method method : oldMethods) {
			...
			if (cfg == null)
				continue;
			headToBody = cfg.syncRemovalHelper();
	...
	private class OuterBuilder {
		...
		private OuterBuilder() {
			...
			for (Inst i : oldCfg.getSuccs(oldHead))
				headSuccs.add(visit(i));
		...
		private Inst visit(Inst oldInst) {
			...
			Set<Inst> oldSuccs;
			int numSuccs;
			if (oldInst instanceof AcqLockInst) {
				InnerBuilder builder =
					new InnerBuilder((AcqLockInst) oldInst, oldMethod);
				newInst = createInvkInst(builder);
				Set<Inst> exits = builder.oldExits;
				numSuccs = exits.size();
				oldSuccs = exits;
				if (numSuccs == 0) {
					// synchronized block contains infinite loop
					if (newTail == null) {
	...
	private class InnerBuilder {
		...
		public InnerBuilder(AcqLockInst oldHead, Method parent) {
            String name = "syncFreeMethod_" + numSyncFreeMethods;
            numSyncFreeMethods++;
            String sign = name + "(args)";
			int lineNum = oldHead.getLineNum();
			newMethod = new Method(sign, type, parent, lineNum);
			newMethod.setStatic();
			Set<Inst> oldBody = headToBody.get(oldHead);
			...
			Var newSyncVar = varMap.get(oldHead.getVar());
			Assertions.Assert(newSyncVar != null);
			...
			Set<Inst> succs = new ArraySet<Inst>();
			newInstToSuccs.put(tmpInst, succs);
			for (Inst inst : oldCfg.getSuccs(oldHead))
				succs.add(visit(inst));
            CFG cfg = new CFG(newHead, newTail, newArgs, newTmps,
            	newRets, newSyncVar, null, newInstToSuccs);
            for (Var var : newArgs)
            	var.setCFG(cfg);
            for (Var var : newTmps)
            	var.setCFG(cfg);
		...
		private Inst visit(Inst oldInst) {
			...
			if (newInst != null)
				return newInst;
			if (oldInst instanceof RelLockInst) {
				if (newTail == null) {
					newTail = new TailInst(-1);
	                newInstToSuccs.put(newTail, emptyInstSet);
				}
			    visited.put(oldInst, newTail);
			    for (Inst oldSucc : oldCfg.getSuccs(oldInst))
			...
			if (oldInst instanceof AcqLockInst) {
				InnerBuilder builder =
					new InnerBuilder((AcqLockInst) oldInst, newMethod);
				newInst = createInvkInst(builder);
				Set<Inst> exits = builder.oldExits;
				oldSuccs = exits;
			} else {
